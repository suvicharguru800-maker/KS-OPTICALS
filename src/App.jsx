...
function CustomersList({ customers, searchQ, setSearchQ, onOpen, onAdd }) {
  ...
  const counts = useMemo(() => {
    const c = { All: customers.length, "Single Vision": 0, "Bifocal": 0, "Progressive": 0 };
    customers.forEach((cust) => {
      const rec = latestRecord(cust);
      if (rec?.lensType && c[rec.lensType] !== undefined) c[rec.lensType]++;
    });
    return c;
  }, [customers]);
  ...
  const filtered = useMemo(() => {
    ...
    return customers.filter((c) => latestRecord(c)?.lensType === catFilter);
  }, [customers, catFilter]);
