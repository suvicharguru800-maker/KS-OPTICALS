...
import {
  ...
  Star, Printer, Link2, LogOut, Mail, Gift, Send, RotateCcw, Image as ImageIcon, Share2, Briefcase, Upload
} from "lucide-react";
...
function rowToCustomer(row) {
  return { id: row.id, name: row.name, mobile: row.mobile, dob: row.dob || "", createdAt: row.created_at, records: row.records || [], source: row.source || "manual" };
}
function customerToRow(customer) {
  return { id: customer.id, name: customer.name, mobile: customer.mobile, dob: customer.dob || "", created_at: customer.createdAt, records: customer.records || [], source: customer.source || "manual" };
}
...
async function uploadOfferImage(file) {
  ...
  if (error) { console.error(error); return { url: null, error: error.message || String(error) }; }
...
const TARGET_CATEGORIES = ["All", "Single Vision", "Bifocal", "Progressive", "VCF"];

/* Parses a .vcf (vCard) export into simple {name, mobile} contacts.
   Only reads the full name (FN) and the first phone number (TEL) of each card —
   everything else in the file is ignored. */
function parseVCF(text) {
  const contacts = [];
  let curName = null, curTel = null;
  text.split(/\r\n|\r|\n/).forEach((line) => {
    if (/^BEGIN:VCARD/i.test(line)) { curName = null; curTel = null; return; }
    if (/^FN[:;]/i.test(line)) {
      const idx = line.indexOf(":");
      if (idx !== -1) curName = line.slice(idx + 1).trim();
      return;
    }
    if (!curTel && /^TEL[:;]/i.test(line)) {
      const idx = line.indexOf(":");
      if (idx !== -1) curTel = line.slice(idx + 1).trim();
      return;
    }
    if (/^END:VCARD/i.test(line)) {
      if (curName && curTel) contacts.push({ name: curName, mobile: curTel });
...
/* Narrows the full customer list down to whichever audience the offer is aimed at. */
function filterByTarget(customers, targetCategory) {
  if (!targetCategory || targetCategory === "All") return customers;
  if (targetCategory === "VCF") return customers.filter((c) => c.source === "vcf");
  return customers.filter((c) => latestRecord(c)?.lensType === targetCategory);
}
...
function computeBatch(offer, customers) {
  const targeted = filterByTarget(customers, offer?.target_category);
  const sorted = [...targeted].sort((a, b) => a.createdAt - b.createdAt);
...
function toCSV(customers) {
  const headers = [
    "Bill ID", "Customer Name", "Mobile", "OD SPH", "OD CYL", "OD Axis",
    "OS SPH", "OS CYL", "OS Axis", "ADD", "Lens Type", "Frame Details", "Lens Details", "Coating",
  ...
  customers.forEach((c) => {
    (c.records || []).forEach((r) => {
      ...
      rows.push([
        r.billId || "", c.name, c.mobile, r.odSph, r.odCyl, r.odAxis,
        r.osSph, r.osCyl, r.osAxis, r.add, r.lensType || "", r.frameDetails, r.lensDetails, r.coating,
...
const PowerGrid = ({ record, compact }) => {
  const Cell = ({ label, value, format = powerStr }) => (
    <div className="text-center">
      <div className="text-[9px] uppercase tracking-widest mb-0.5" style={{ color: COLORS.muted }}>{label}</div>
      <div className="ks-mono font-medium" style={{ fontSize: compact ? 13 : 15, color: COLORS.ink }}>{format(value)}</div>
    </div>
  );
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${COLORS.line}` }}>
      ...
      <div className="grid grid-cols-4 items-center border-t" style={{ borderColor: COLORS.line }}>
        ...
        <Cell label="" value={record.odSph} /><Cell label="" value={record.odCyl} /><Cell label="" value={record.odAxis} format={axisStr} />
      </div>
      <div className="grid grid-cols-4 items-center border-t" style={{ borderColor: COLORS.line }}>
        ...
        <Cell label="" value={record.osSph} /><Cell label="" value={record.osCyl} /><Cell label="" value={record.osAxis} format={axisStr} />
      </div>
      {record.add ? (
...
/* ---- Encode/decode the current screen into the URL hash, so refreshing the page
   (not just pressing back) lands back on the same screen instead of resetting to Dashboard. ---- */
function navStateToHash(state) {
  return "#nav=" + encodeURIComponent(JSON.stringify(state));
}
function hashToNavState() {
...
export default function App() {
  ...
  const isBillRoute = window.location.pathname.startsWith("/bill/");
  const initialNav = isBillRoute ? null : hashToNavState();
  const [view, setView] = useState(initialNav?.view || "dashboard");
  const [activeId, setActiveId] = useState(initialNav?.activeId || null);
  const [editingRecordId, setEditingRecordId] = useState(initialNav?.editingRecordId || null);
  const [toast, setToast] = useState(null);
  const [confirmState, setConfirmState] = useState(null);
  const [searchQ, setSearchQ] = useState("");
  const [billCtx, setBillCtx] = useState(initialNav?.billCtx || null);
  const [offer, setOffer] = useState(null);
  const [customerCatFilter, setCustomerCatFilter] = useState(initialNav?.customerCatFilter || "All");
  ...
  /* ---- Make the Android/browser back button navigate within the app instead of closing it ----
     We keep the current screen (view + which record/offer/etc. is open) mirrored into browser
     history. Pressing back then just pops one of our own history entries, until the very first
     screen (Dashboard) is reached — only then does back fall through to the OS (minimize/close),
     which is the expected behavior at the root of an app. */
  const isPoppingRef = useRef(false);
  const historyInitRef = useRef(false);
  useEffect(() => {
    if (!dataReady || publicToken) return;
    const state = { view, activeId, editingRecordId, billCtx, customerCatFilter };
  ...
  }, [view, activeId, editingRecordId, billCtx, customerCatFilter, dataReady, publicToken]);

  useEffect(() => {
    const onPopState = (e) => {
      if (!e.state) return; // nothing of ours left — let the OS handle it (e.g. close the app)
      isPoppingRef.current = true;
      setView(e.state.view || "dashboard");
      setActiveId(e.state.activeId || null);
      setEditingRecordId(e.state.editingRecordId || null);
      setBillCtx(e.state.billCtx || null);
      setCustomerCatFilter(e.state.customerCatFilter || "All");
    };
  ...
  const saveOffer = async (form, { resetCycle }) => {
    const next = {
      ...
      target_category: form.targetCategory || "All",
  ...
  const filtered = useMemo(() => {
    ...
    return customers.filter((c) => {
      ...
      return (c.records || []).some((r) => {
        const fields = [r.odSph, r.odCyl, r.odAxis, r.osSph, r.osCyl, r.osAxis, r.add, r.product, r.frameDetails, r.lensDetails, r.billId, r.lensType];
  ...
  // The public /bill/:token page reads from a separate "bills" snapshot row (so a customer can
  // view it without login, and without exposing the rest of the customer's data). Whenever the
  // underlying record — or the customer's name/mobile — changes after a bill was generated, we
  // re-save that snapshot so the already-shared link stays in sync instead of going stale.
  const buildBillSnapshot = (cust, rec) => ({
    billId: rec.billId, billDate: rec.billDate, customerName: cust.name, mobile: cust.mobile,
  ...
  const syncBillSnapshot = async (cust, rec) => {
    if (rec?.billId && rec?.billToken) await savePublicBillRow(rec.billToken, buildBillSnapshot(cust, rec));
  };
  ...
  const updateCustomerInfo = async (id, info) => {
    ...
    // Keep any already-shared bill links showing the corrected name/mobile too
    await Promise.all((updated.records || []).filter((r) => r.billToken).map((r) => syncBillSnapshot(updated, r)));
  ...
  const updateRecord = async (customerId, recordId, form) => {
    ...
    await saveCustomerRow(updated);
    setCustomers((prev) => prev.map((c) => (c.id === customerId ? updated : c)));
    // If this record already has a live bill link, refresh it with the edited details
    const newRec = records.find((r) => r.id === recordId);
    await syncBillSnapshot(updated, newRec);
  ...
  const bulkDeleteCustomers = async (ids) => {
    await Promise.all(ids.map((id) => deleteCustomerRow(id)));
    setCustomers((prev) => prev.filter((c) => !ids.includes(c.id)));
    showToast(`Deleted ${ids.length} customer${ids.length !== 1 ? "s" : ""}`);
  };
  ...
  const requestBulkDelete = (ids) => {
    ...
    setConfirmState({
      ...
      body: "Every selected customer and their entire record history will be permanently deleted. This cannot be undone.",
      onCancel: () => setConfirmState(null),
      onConfirm: async () => { await bulkDeleteCustomers(ids); setConfirmState(null); },
    });
  };

  const importVCF = async (file) => {
    const text = await file.text();
    const parsed = parseVCF(text);
    if (parsed.length === 0) { showToast("No contacts found in that file"); return; }
    const existingMobiles = new Set(customers.map((c) => normalizeMobile(c.mobile)));
    const seen = new Set();
    const now = Date.now();
    const newCustomers = [];
    let skipped = 0;
    parsed.forEach((p) => {
      const digits = normalizeMobile(p.mobile);
      if (digits.length !== 10 || existingMobiles.has(digits) || seen.has(digits)) { skipped++; return; }
      seen.add(digits);
      newCustomers.push({
        id: uid(), name: (p.name || "Unknown").trim(), mobile: formatMobileFull(digits),
        dob: "", createdAt: now, records: [], source: "vcf",
      });
    });
    await Promise.all(newCustomers.map((c) => saveCustomerRow(c)));
    setCustomers((prev) => [...newCustomers, ...prev]);
    showToast(`Imported ${newCustomers.length} contact${newCustomers.length !== 1 ? "s" : ""}${skipped ? ` · skipped ${skipped}` : ""}`);
  };
          ...
          <div className="max-w-4xl mx-auto px-4 md:px-8 py-5 md:py-8">
            ...
            {view === "customers" && (
              <CustomersList customers={filtered} searchQ={searchQ} setSearchQ={setSearchQ}
                ...
                catFilter={customerCatFilter} setCatFilter={setCustomerCatFilter}
...
function CustomerRow({ customer, onClick, selectMode, selected, onToggleSelect }) {
  const rec = latestRecord(customer);
  const handleClick = () => {
    if (selectMode) onToggleSelect(customer.id);
    else onClick();
  };
  return (
    <div onClick={handleClick} className="rounded-xl px-4 py-3.5 flex items-center justify-between cursor-pointer transition-colors"
      style={{ background: selected ? COLORS.cream : COLORS.paper, border: `1px solid ${selected ? COLORS.brass : COLORS.line}` }}>
      <div className="flex items-center gap-3 min-w-0">
        {selectMode && (
          <div className="flex items-center justify-center rounded-full shrink-0" style={{
            width: 22, height: 22, border: `1.5px solid ${selected ? COLORS.brass : COLORS.line}`,
        ...
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            <span className="text-xs" style={{ color: COLORS.muted }}>{customer.mobile}</span>
            {rec?.lensType && (
              <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: COLORS.cream, color: COLORS.brass }}>
                {rec.lensType}
              </span>
            )}
          </div>
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        {!selectMode && <ChevronRight size={16} style={{ color: COLORS.muted }} />}
      </div>
    </div>
  );
}
...
/* ================= CUSTOMERS LIST ================= */
function CustomersList({ customers, searchQ, setSearchQ, onOpen, onAdd, onImportVCF, catFilter, setCatFilter, onRequestBulkDelete }) {
  ...
  const counts = useMemo(() => {
    const c = {};
    TARGET_CATEGORIES.forEach((t) => { c[t] = filterByTarget(customers, t).length; });
    return c;
  }, [customers]);

  const filtered = useMemo(() => filterByTarget(customers, catFilter), [customers, catFilter]);

  // Drop any selected ids that no longer exist in the (possibly filtered) list — e.g. after a delete
  useEffect(() => {
    setSelectedIds((prev) => {
    ...
    <div className="ks-rise">
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-2">
          {!selectMode ? (
            <>
              <input ref={fileInputRef} type="file" accept=".vcf,text/vcard,text/x-vcard" className="hidden" onChange={handleFile} />
              <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-semibold"
                style={{ border: `1px solid ${COLORS.line}`, color: COLORS.ink }}>
                <Upload size={14} /> <span className="hidden sm:inline">Import VCF</span>
              </button>
              <button onClick={() => setSelectMode(true)} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-semibold"
                style={{ border: `1px solid ${COLORS.line}`, color: COLORS.ink }}>
                <Check size={14} /> <span className="hidden sm:inline">Select</span>
              </button>
            ...
            <button onClick={exitSelectMode} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-semibold"
              style={{ border: `1px solid ${COLORS.line}`, color: COLORS.ink }}>
              <X size={14} /> Cancel
            </button>
          )}
      ...
      {selectMode && (
        <div className="flex items-center justify-between mb-4 rounded-xl px-4 py-3" style={{ background: COLORS.ink }}>
          <button onClick={toggleSelectAll} className="text-xs font-semibold" style={{ color: COLORS.brassLight }}>
            {allFilteredSelected ? "Deselect All" : `Select All (${filtered.length})`}
          </button>
          <span className="text-xs" style={{ color: "#B5B2AA" }}>{selectedIds.size} selected</span>
          <button onClick={() => onRequestBulkDelete([...selectedIds])} disabled={selectedIds.size === 0}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold"
            style={{ background: selectedIds.size ? COLORS.danger : "rgba(255,255,255,0.15)", color: "#fff", opacity: selectedIds.size ? 1 : 0.5 }}>
            <Trash2 size={13} /> Delete
          </button>
        </div>
      )}

      <div className="relative mb-3 sticky top-0 z-10 pt-1 pb-1" style={{ background: COLORS.cream }}>
        <Search size={17} className="absolute left-3.5 top-1/2 -translate-y-1/2" style={{ color: COLORS.muted }} />
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto pb-1" style={{ WebkitOverflowScrolling: "touch" }}>
        {TARGET_CATEGORIES.map((t) => {
          const active = catFilter === t;
          const label = t === "All" ? "All" : t === "VCF" ? "VCF" : t;
          return (
            <button key={t} onClick={() => setCatFilter(t)}
              ...
              {label} <span style={{ opacity: 0.7 }}>({counts[t] || 0})</span>
            </button>
          );
        })}
      </div>

      <div className="text-xs mb-3" style={{ color: COLORS.muted }}>{filtered.length} result{filtered.length !== 1 ? "s" : ""}</div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {filtered.map((c) => (
            <CustomerRow key={c.id} customer={c} onClick={() => onOpen(c.id)}
              selectMode={selectMode} selected={selectedIds.has(c.id)} onToggleSelect={toggleSelect} />
          ))}
        </div>
      )}
    </div>
...
const emptyRecordForm = {
  ...
  lensType: "", product: "", frameDetails: "", lensDetails: "", coating: "",
  orderDate: "", deliveryDate: "", billDate: "",
  totalPrice: "", discount: "", paymentStatus: "Paid", notes: "",
  billId: "", billToken: "",
};
...
function LensTypeField({ rec, upd }) {
  return (
    <FormSection label="Lens Type">
      <div className="grid grid-cols-3 gap-2">
        {LENS_TYPES.map((t) => {
          const selected = rec.lensType === t;
          return (
            <button type="button" key={t} onClick={() => upd("lensType", t)}
              className="flex items-center justify-center gap-1.5 py-2.5 px-1 rounded-lg text-xs font-semibold text-center transition-colors"
              style={{
...
function EditCustomerInfo({ customer, onCancel, onSave }) {
  ...
  const save = () => {
    if (!name.trim()) return setErr("Customer name is required");
    if (normalizeMobile(mobile).length !== 10) return setErr("Enter a valid 10-digit mobile number");
    onSave({ name: name.trim(), mobile: formatMobileFull(mobile), dob: customer.dob || "" });
  };
  return (
            ...
            <SectionCard key={r.id}>
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  {r.lensType && (
                    <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ background: COLORS.cream, color: COLORS.brass }}>{r.lensType}</span>
                  )}
                </div>
                <div className="flex items-center gap-3">
...
function buildBillPDF(customer, r) {
  ...
  const odVals = [powerStr(r.odSph), powerStr(r.odCyl), axisStr(r.odAxis)];
  const osVals = [powerStr(r.osSph), powerStr(r.osCyl), axisStr(r.osAxis)];
  ...
  sectionLabel("Lens Type", y); y += 14;
  const lensBoxSize = 10;
  const lensColW = contentW / 3;
  LENS_TYPES.forEach((t, i) => {
    const bx = marginX + i * lensColW;
    const checked = r.lensType === t;
    doc.setDrawColor(checked ? brass : muted); doc.setLineWidth(0.9);
    if (checked) { doc.setFillColor(brass); doc.rect(bx, y, lensBoxSize, lensBoxSize, "FD"); }
    else { doc.rect(bx, y, lensBoxSize, lensBoxSize, "D"); }
    if (checked) {
      doc.setDrawColor(255, 255, 255); doc.setLineWidth(1.2);
      doc.line(bx + 2, y + 5, bx + 4, y + 7.5);
      doc.line(bx + 4, y + 7.5, bx + 8, y + 2.5);
    }
    doc.setFont("helvetica", checked ? "bold" : "normal"); doc.setFontSize(9.5); doc.setTextColor(checked ? ink : muted);
    doc.text(t, bx + lensBoxSize + 5, y + lensBoxSize - 1.5);
  });
  y += lensBoxSize + 16;
...
function BillCard({ billId, billDate, customerName, mobile, record: r, finalAmount, status }) {
  return (
    <div className="ks-bill-print rounded-2xl p-6 md:p-8" style={{ background: COLORS.paper, border: `1px solid ${COLORS.line}` }}>
      ...
      <div className="mb-5">
        <div className="text-[11px] uppercase tracking-widest font-bold mb-2" style={{ color: COLORS.brass }}>Lens Type</div>
        <div className="grid grid-cols-3 gap-2">
          {LENS_TYPES.map((t) => {
            const checked = r.lensType === t;
            return (
              <div key={t} className="flex items-center gap-1.5 px-2 py-2 rounded-lg" style={{ border: `1px solid ${COLORS.line}`, background: checked ? COLORS.cream : "transparent" }}>
                <div className="flex items-center justify-center rounded shrink-0" style={{ width: 15, height: 15, border: `1.5px solid ${checked ? COLORS.brass : COLORS.muted}`, background: checked ? COLORS.brass : "transparent" }}>
                  {checked && <Check size={11} style={{ color: "#fff" }} />}
                </div>
                <span className="text-[11px] font-medium" style={{ color: checked ? COLORS.ink : COLORS.muted }}>{t}</span>
              </div>
            );
          })}
        </div>
...
function OffersPage({ offer, batchInfo, onSave, onResetCycle, onBack, onOpenBatch }) {
  ...
  const [targetCategory, setTargetCategory] = useState(offer?.target_category || "All");
  ...
  const handleImagePick = async (e) => {
    ...
    if (url) setImageUrl(url); else setErr(`Image upload failed: ${error || "unknown error"}`);
    setUploading(false);
  };
  ...
  const save = (resetCycle) => {
    ...
    setErr("");
    onSave({ title, description, batchSize: Number(batchSize) || 30, imageUrl, targetCategory }, { resetCycle });
  };
  ...
  return (
    <div className="ks-rise">
        ...
        <SectionCard className="mb-5">
          ...
          <div className="rounded-lg px-3.5 py-2.5 mb-3" style={{ background: COLORS.cream }}>
            ...
            <div className="text-xs mt-1" style={{ color: COLORS.muted }}>
              Audience: <span style={{ color: COLORS.ink, fontWeight: 600 }}>{offer.target_category && offer.target_category !== "All" ? offer.target_category : "All Customers"}</span>
            </div>
          </div>
      ...
      <FormSection label={offer ? "Edit Offer" : "Create Offer"}>
        ...
        <Field label="Send To" hint="Which customers should get this offer">
          <div className="grid grid-cols-2 gap-2">
            {TARGET_CATEGORIES.map((t) => {
              const selected = targetCategory === t;
              return (
                <button type="button" key={t} onClick={() => setTargetCategory(t)}
                  className="py-2.5 px-1 rounded-lg text-xs font-semibold text-center"
                  style={{
                    background: selected ? COLORS.ink : COLORS.paper,
                    color: selected ? "#fff" : COLORS.ink,
                    border: `1px solid ${selected ? COLORS.ink : COLORS.line}`,
                  }}>
                  {t === "All" ? "All Customers" : t === "VCF" ? "VCF Contacts" : t}
                </button>
              );
            })}
          </div>
