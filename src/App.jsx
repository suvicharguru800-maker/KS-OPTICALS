...
export default function App() {
  ...
  useEffect(() => {
    ...
    const state = { view, activeId, editingRecordId, billCtx };
  ...
  }, [view, activeId, editingRecordId, billCtx, dataReady, publicToken]);
...
/* ================= CUSTOMERS LIST ================= */
function CustomersList({ customers, searchQ, setSearchQ, onOpen, onAdd, onImportVCF }) {
  ...
  return (
    <div className="ks-rise">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {filtered.map((c) => <CustomerRow key={c.id} customer={c} onClick={() => onOpen(c.id)} />)}
        </div>
      )}
    </div>
  );
