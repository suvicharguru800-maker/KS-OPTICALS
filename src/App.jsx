...
import {
  ...
  Star, Printer, Link2, LogOut, Mail
} from "lucide-react";
...
function CustomerProfile({ customer, onBack, onEditInfo, onAddRecord, onEditRecord, onDeleteRecord, onDeleteCustomer, onGenerateBill }) {
  ...
  const waLink = `https://wa.me/${mobileDialDigits(customer.mobile)}`;
    ...
    <div className="ks-rise">
      ...
      <SectionCard className="mb-4">
        ...
        <div className="grid grid-cols-2 gap-2.5 mb-2.5">
          ...
          <a href={waLink} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-semibold"
...
function buildBillPDF(customer, r) {
  ...
  center("SEE BETTER  ·  LOOK BETTER  ·  LIVE BETTER", y, 8.5, "normal", brass); y += 18;
...
function BillPage({ customer, record: r, onBack, showToast }) {
  ...
  const shareLinkToWhatsApp = () => {
    window.open(`https://wa.me/${mobileDialDigits(customer.mobile)}?text=${encodeURIComponent(waMessage(customer.name, billLink))}`, "_blank");
  };
  ...
  const shareToWhatsApp = async () => {
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        document.body.appendChild(a); a.click(); document.body.removeChild(a);
        URL.revokeObjectURL(url);
        window.open(`https://wa.me/${mobileDialDigits(customer.mobile)}?text=${encodeURIComponent(text)}`, "_blank");
...
/* ================= SETTINGS ================= */
function SettingsPage({ customers, totalRecords, onExport, onChangePin, userEmail }) {
  return (
    <div className="ks-rise">
